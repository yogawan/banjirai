...
const ChatHistory = memo(({ chatHistory, isLoading, handleClearHistory }) => {
  return (
    <div className="flex flex-col min-h-full pb-36 md:pb-44">
      {/* Empty State */}
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center space-y-4 p-6">
            <div className="w-16 h-16 mx-auto bg-gradient-to-br from-blue-500/20 to-purple-500/20 rounded-full flex items-center justify-center mb-4">
              <Icon icon="material-symbols:chat-bubble-outline" className="text-2xl text-white/60" />
            </div>
            <div className="space-y-2">
              <h3 className="text-sm font-medium text-white/80">Mulai Percakapan Baru</h3>
              <p className="text-xs leading-relaxed text-white/50 max-w-sm mx-auto">
                Jika ada pesan yang tidak sepantasnya,{' '}
                <a href="https://github.com/yogawan/jawiraiv1.6.3" className="underline hover:text-white/70 transition-colors duration-200" target="_blank" rel="noopener noreferrer">
      ...
      {isLoading && (
        <div className="flex items-start gap-3 px-4 py-2 animate-fade-in">
          <div className="flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-blue-500/20 to-purple-500/20 flex items-center justify-center">
            <Icon icon="line-md:loading-twotone-loop" className="text-sm text-white/70" />
          </div>
      ...
      {chatHistory.length > 0 && (
        <div className="flex justify-center px-4 py-6 border-t border-white/5">
          ...
          >
            <Icon icon="material-symbols:delete-outline" className="text-sm group-hover:text-red-400 transition-colors duration-300" />
