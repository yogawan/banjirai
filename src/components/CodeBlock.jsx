    ...
    <div className={`w-full max-w-4xl mx-auto ${className}`}>
      {parts.map((part, index) => {
        if (part.startsWith('```')) {
          ...
          const displayCode = shouldTruncate && !isExpanded ? codeLines.slice(0, 15).join('\n') + '\n...' : code;

          return (
            <div key={index} className="relative group border border-white/10 rounded-2xl mb-6 overflow-hidden bg-gradient-to-br from-gray-900/50 to-black/50 backdrop-blur-sm hover:border-white/20 transition-all duration-300">
              {/* Header */}
              <div className="flex items-center justify-between px-4 py-3 bg-black/30 border-b border-white/10">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-white/40 text-xs">{codeLines.length} lines</div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-medium transition-all duration-200 ${
                      showLineNumbers ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : 'bg-white/5 text-white/60 border border-white/10 hover:bg-white/10'
                    }`}
                  ...
                  {shouldTruncate && (
                    <button onClick={() => toggleExpanded(index)} className="px-3 py-1 bg-white/5 text-white/60 text-xs rounded-lg border border-white/10 hover:bg-white/10 hover:text-white/80 transition-all duration-200">
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-medium transition-all duration-200 ${
                      copiedIndex === index ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-white/5 text-white/60 border border-white/10 hover:bg-white/10 hover:text-white/80'
                    }`}
                  >
              ...
              <div className="relative overflow-x-auto">
                ...
                {shouldTruncate && !isExpanded && <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-gray-900 to-transparent pointer-events-none"></div>}
              </div>
      ...
      {text.split('\n').map((line, index) => {
        ...
        if (/^\s*[-*+]\s/.test(line)) {
          ...
          return (
            <div key={index} className="flex items-start gap-3 mb-2 group">
              ...
              <div className="flex-1" dangerouslySetInnerHTML={{ __html: formatText(listContent) }} />
            </div>
          );
        }
        ...
        if (/^\s*\d+\.\s/.test(line)) {
          ...
          if (match) {
            ...
            return (
              <div key={index} className="flex items-start gap-3 mb-2 group">
                <span className="text-blue-400 font-mono text-sm mt-1 group-hover:text-blue-300 transition-colors duration-200">{number}.</span>
                <div className="flex-1" dangerouslySetInnerHTML={{ __html: formatText(content) }} />
              </div>
            );
          }
        }
        ...
        if (/^#{1,6}\s/.test(line)) {
          ...
          return <h1 key={index} className={headerClasses[level]} dangerouslySetInnerHTML={{ __html: formatText(content) }} />;
        }
        ...
        if (/^\s*>\s/.test(line)) {
          ...
          return (
            <blockquote key={index} className="border-l-4 border-blue-500/50 pl-4 py-2 my-3 bg-blue-500/5 rounded-r-lg">
              <div className="text-white/90 italic" dangerouslySetInnerHTML={{ __html: formatText(content) }} />
            </blockquote>
          );
        }
        ...
        // Regular paragraphs
        if (line.trim()) {
          return <p key={index} className="mb-3 leading-relaxed hover:text-white/90 transition-colors duration-200" dangerouslySetInnerHTML={{ __html: formattedLine }} />;
        }
